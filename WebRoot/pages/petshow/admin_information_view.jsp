	...
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
  			<div class="form-group">
		    	用户名 <input type="text" style="width:120px;" class="form-control" name="Q_LKS_adminname">
		  	</div>		  	
		  	<div class="form-group">
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			colModel: [
				...
				{label: '用户名', name: 'adminname', width: 60},
				{label: '姓名', name: 'realName', width: 50},
				{label: '性别', name: 'sex.itemValue', width: 50},
				{label: '角色', name: 'roleName', width: 50},
		...
		function addfunc() {
			...
			layer.open({
				...
				area: ['600px', '480px'],
				maxmin: true,
			    content: ['pages/petshow/admin_information_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./admin/save', form.serialize(), function(json){
			    		if(json.success) {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./admin/delete' ,{ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
