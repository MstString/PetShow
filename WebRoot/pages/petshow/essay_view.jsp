	...
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
  			<div class="form-group">
		    	文章名称 <input type="text" style="width:120px;" class="form-control" name="Q_LKS_title">
		  	</div>		  	
		  	<div class="form-group">
		    	时间<input type="text" style="width:120px;" height="100%" class="form-control" name="Q_LKS_publish_time">
		  	</div>
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			colModel: [
				...
				{label: '文章名称', name: 'title', width: 100},
				{label: '图片路径', name: 'essay_picture_path', width: 240}
			],
			pager: '#pager',
			multiboxonly: true,
		...
		function addfunc() {
			...
			layer.open({
				...
				area: ['600px', '480px'],
				maxmin: true,
			    content: ['pages/petshow/essay_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./essay/save', form.serialize(), function(json){
			    		if(json.success) {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./essay/delete' ,{ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
