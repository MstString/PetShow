	...
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
  			<div class="form-group">
		    	账号 <input type="text" style="width:120px;" class="form-control" name="Q_LKS_editorname">
		  	</div>		  	
		  	<div class="form-group">
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			url: 'editor/self_list',
			caption: '<i class="glyphicon glyphicon-list"></i> 编辑列表',
			colModel: [
				...
				{label: '账号', name: 'editorname', width: 100},
				{label: '姓名', name: 'realName', width: 100},
				{label: '角色', name: 'roleName', width: 100},
				{label: '性别', name: 'sex.itemValue', width: 100},
				{label: '头像路径', name: 'head_portrait_path', width: 240}
			],
			multiboxonly: true,
			ondblClickRow: editfunc
		});
		grid.navGrid('#pager',
			{editfunc: editfunc, delfunc:delfunc, search: false,
			edittext:'编辑', deltext:'删除', refreshtext:'刷新'}
		);
		...
		function addfunc() {
			...
			layer.open({
				...
				area: ['600px', '480px'],
				maxmin: true,
			    content: ['pages/petshow/self_editor_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./editor/save', form.serialize(), function(json){
			    		if(json.success) {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./editor/delete' ,{ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
