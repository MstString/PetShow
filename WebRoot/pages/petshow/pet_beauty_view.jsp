	...
	<div class="row">
		...
		<form id="form-search" class="form-inline" method="post">
  			<div class="form-group">
		    	美容院名称 <input type="text" style="width:120px;" class="form-control" name="Q_LKS_beauty_name">
		  	</div>		  	
		  	<div class="form-group">
...
<script type="text/javascript">
	...
	$(function() {
		...
		grid.jqGrid({
			...
			url: 'pet_beauty/list',
			caption: '<i class="glyphicon glyphicon-list"></i> 管理员列表',
			colModel: [
				...
				{label: '宠物美容院名称', name: 'beauty_name', width: 100},
				{label: '地址', name: 'address', width: 150},
				{label: '电话', name: 'phonenum', width: 50},
				{label: '图片路径', name: 'beauty_picture_path', width: 240}
			],
			pager: '#pager',
			multiboxonly: true,
		...
		function addfunc() {
			...
			layer.open({
				...
				area: ['600px', '480px'],
				maxmin: true,
			    content: ['pages/petshow/pet_beauty_form.jsp', 'yes'],
			    btn: ['保存', '关闭'],
			    btn1: function(index, layero) {
			    	var form = layer.getChildFrame('form', index);
			    	$.post('./pet_beauty/save', form.serialize(), function(json){
			    		if(json.success) {
	...
	function delfunc(rowid) {
		layer.confirm('确定删除该条记录吗？', {icon:3, title:'温馨提示'}, function(index) {
			$.getJSON('./pet_beauty/delete' ,{ids: rowid+''}, function(json) {
				if(json.success) {
					grid.trigger("reloadGrid", [{page: 1}]);
